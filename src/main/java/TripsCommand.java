    ...
    public String execute(Graph graph) {
        ...
        if (parameters.length < 4) {
            throw new IllegalArgumentException(Application.getProperty("invalid_command") + ": " + toString() +
                    Application.getProperty("incomplete_command"));
        }
        ...
        if (nodes.length != 2) {
            throw new IllegalArgumentException(Application.getProperty("invalid_command") + ": " + toString() +
                    Application.getProperty("must_specify_source_and_target"));
        }
        ...
        Path[] paths = new PathBuilder(runCond, filterCond, false).buildAllPaths(graph, start, target);
