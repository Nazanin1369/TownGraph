...
public class PathBuilder {
     ...
     */
    public PathBuilder(Condition runCond, Condition filterCond, boolean stopOnTarget) {
     ...
     * @return
     */
    public Path[] buildAllPaths(Graph graph, Node start, Node end) {
        return findPaths(graph, start, end, new Path(0,0)).toArray(new Path[0]);
    }

    private ArrayList<Path> findPaths(Graph graph, Node current,  Node end, Path ancestor) {
        ...
        for (Edge edge: current.getAdjacents()) {
            ...
            for (Path path: findPaths(graph, destination, end, newPath)) {
                paths.add(path);
            }
        }
        return paths;
